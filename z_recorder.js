function p5recorder(numFrames, filename, delay, repeat, buffersPerFrame) {
  ...
  this.addBuffer = function() {
    ...
    if (this.framesRecorded < this.numFrames) {
      ...
      this.offscreenContext.drawImage(the_canvas, 0, 0, width, height);
      ...
      if (this.buffersPerFrame > 1) {
        ...
        if(this.buffersRecorded == this.buffersPerFrame) {
          ...
          */
          ...
          //new code
        //  debugger;/*
          var glpix = new Uint8Array(960 * 480 * 4);
          gc.readPixels(0,0, 960, 480, gc.RGBA, gc.UNSIGNED_BYTE, glpix);
          var target = this.offscreenContext.createImageData(960,480);
          target.data.set(glpix);
          this.offscreenContext.putImageData(target,0,0);
